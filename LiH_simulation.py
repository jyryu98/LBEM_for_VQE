...
def experiment(num_trunc_T, num_trunc_P, em_instance, ef_instance, angles):
    group_pauli_op, [ansatz,num_par_gates] = main()
    print('Ansatz & qubit hamiltonian created')
    trunc_T, trunc_P = truncate_training_set(num_par_gates, num_trunc_P, num_trunc_T)
...
for p in P:
    ef_expval, em_expval, n_expval = experiment(3*p, p, em_instance, ef_instance, angles)
